...
use F4\Pechkin\DataType\InputRichBlockCollage;
...
final class InputRichBlockCollageTest extends TestCase
{
    ...
    {
        $data = $this->loadFixture('input_rich_block_collage_full.json');
        $obj = InputRichBlockCollage::fromArray($data);

        $this->assertInstanceOf(InputRichBlockCollage::class, $obj);
    ...
    {
        $data = $this->loadFixture('input_rich_block_collage_minimal.json');
        $obj = InputRichBlockCollage::fromArray($data);
